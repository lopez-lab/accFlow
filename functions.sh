...
function email_sbatch {
	...
	sed "s/JOBID/$jobid/g" $ACCFLOW_TOOLS/templates/email.sbatch | sed "s/EMAIL/$DEFAULT_EMAIL/g" | sed "s/JOBNAME/$title/g" | sed "s/PARTITION/$DEFAULT_PARTITION/g" | sbatch 1>/dev/null
}
...
function restart_opt {
    ...
    python $ACCFLOW/scripts/restart-g16.py $input_file
...
function update_existing_flow {
	...
	source_config
	cd $ACCFLOW_TOOLS
	rsync -r --exclude=setup-flow.sh --exclude=test --exclude=functions.sh --exclude=LICENSE --exclude=README.md $ACCFLOW/* .
	cd $cur_dir
}
...
function extract_data {
	python $ACCFLOW/utils/data_extractor.py
}
...
function setup_freq {
	...
	# setup freq job
	bash $ACCFLOW_TOOLS/scripts/make-com.sh -i="$log_file" -r="$new_route" -c="$charge" -s="$mult" -t="$freq" -l="../freq_calcs/"
	cd "../freq_calcs/"

	# setup sbatch
	cp $ACCFLOW_TOOLS/templates/freq_sbatch.txt $freq.sbatch
...
function setup_vee {
	...
	local log_file=$1
        ...
        # setup vee job
        bash $ACCFLOW_TOOLS/scripts/make-com.sh -i="$log_file" -r="$new_route" -c="$charge" -s="$mult" -t="$vee" -l="../tddft/"
        cd "../tddft/"

        # setup sbatch
        cp $ACCFLOW_TOOLS/templates/tddft_sbatch.txt $tddft.sbatch
...
# use: setup-vee <log file>
# effect: creates an input and sbatch file for a vee job and submits it
function setup_single_point {
        source_config
        local log_file=$1
        local com_file="${log_file/.log/.com}"
        local route=$(grep '#p' $com_file | sed "s|B3LYP/4-31G\*|CAM-B3LYP/6-31+G(d)|" | sed "s|EmpiricalDispersion=GD3BJ| |")
        local opt_keyword=$(echo $route | awk '/opt/' RS=" ")
        local charge=$(grep 'Charge =' $log_file | awk '{print $3}')
    local mult=$(grep 'Charge =' $log_file | awk '{print $6}')
        local sp="${log_file/.log/_sp}"
        local new_route=$(echo $route | sed "s|$opt_keyword| |")

        # setup vee job
        bash $ACCFLOW_TOOLS/scripts/make-com.sh -i="$log_file" -r="$new_route" -c="$charge" -s="$mult" -t="$sp" -l="../sp/"
        cd "../sp/"

        # setup sbatch
        cp $ACCFLOW_TOOLS/templates/sp_sbatch.txt $sp.sbatch
        sed -i "s/JOBNAME/$sp/g" $sp.sbatch
        sed -i "s/EMAIL/$DEFAULT_EMAIL/" $sp.sbatch
        sed -i "s/TIME/$DFT_TIME/" $sp.sbatch
        sed -i "s/PARTITION/$DEFAULT_PARTITION/" $sp.sbatch
        sbatch $sp.sbatch
...
function resubmit_array {
    ...
    if [[ "$curr_dir" == "$S0_VAC" ]]; then
        jobid=$(submit_array "$TITLE\_S0_VAC" "g16_inp.txt" "com" "$ACCFLOW_TOOLS/templates/array_g16_s0_dft-opt_vac.sbatch" "$DFT_TIME")
		sed "s/S0_DFT_OPT_VAC_ID/$jobid/g" $ACCFLOW_TOOLS/templates/dft-opt_submitter.sbatch | sbatch
	elif [[ "$curr_dir" == "$S0_SOLV" ]]; then
		jobid=$(submit_array "$TITLE\_S0_SOLV" "g16_inp.txt" "com" "$ACCFLOW_TOOLS/templates/array_g16_dft-opt.sbatch" "$DFT_TIME")
	elif [[ "$curr_dir" == "$S1_SOLV" ]]; then
		jobid=$(submit_array "$TITLE\_S1_SOLV" "g16_inp.txt" "com" "$ACCFLOW_TOOLS/templates/array_g16_dft-opt.sbatch" "$DFT_TIME")
	elif [[ "$curr_dir" == "$T1_SOLV" ]]; then
        jobid=$(submit_array "$TITLE\_T1_SOLV" "g16_inp.txt" "com" "$ACCFLOW_TOOLS/templates/array_g16_dft-opt.sbatch" "$DFT_TIME")
	elif [[ "$curr_dir" == "$CAT_RAD_VAC" ]]; then
        jobid=$(submit_array "$TITLE\_CAT-RAD_VAC" "g16_inp.txt" "com" "$ACCFLOW_TOOLS/templates/array_g16_dft-opt.sbatch" "$DFT_TIME")
	elif [[ "$curr_dir" == "$CAT_RAD_SOLV" ]]; then
        jobid=$(submit_array "$TITLE\_CAT-RAD_SOLV" "g16_inp.txt" "com" "$ACCFLOW_TOOLS/templates/array_g16_dft-opt.sbatch" "$DFT_TIME")
	elif [[ "$curr_dir" == "$SP_TDDFT" ]]; then
		jobid=$(submit_array "$TITLE\_SP-TDDFT" "g16_inp.txt" "com" "$ACCFLOW_TOOLS/templates/array_g16_sp_td-dft.sbatch" "$DFT_TIME")
	elif [[ "$curr_dir" == "$PM7" ]]; then
		jobid=$(submit_array "$TITLE\_PM7" "g16_inp.txt" "com" "$ACCFLOW_TOOLS/templates/array_g16_pm7.sbatch" "$PM7_TIME")
	elif [[ "$curr_dir" == "$RM1_D" ]]; then
		jobid=$(submit_array "$TITLE\_RM1-D" "gamess_inp.txt" "inp" "$ACCFLOW_TOOLS/templates/array_gamess_rm1-d.sbatch" "$DFT_TIME")
		sed "s/RM1_ID/$jobid/g" $ACCFLOW_TOOLS/templates/sp-dft_submitter.sbatch | sbatch
	elif [[ "$curr_dir" == "$SP_DFT" ]]; then
		jobid=$(submit_array "$TITLE\_SP-DFT" "g16_inp.txt" "com" "$ACCFLOW_TOOLS/templates/array_g16_sp-dft.sbatch" "$DFT_TIME")
		LEE_ID=$(sed "s/SP_DFT_ID/$jobid/" $ACCFLOW_TOOLS/templates/least-energy-extractor.sbatch | sed "s/EMAIL/$DEFAULT_EMAIL/" | sbatch)
		cd $MAIN_DIR; DFT_VEE_ID=$(sed "s/LEE_ID/$LEE_ID/g" $ACCFLOW_TOOLS/templates/dft-vee_submitter.sbatch | sbatch); cd $curr_dir
	fi
...
# use: update_flow
# effect: Updates $ACCFLOW directory to match GitHub repository
...
function begin_calcs {
	...
	local curr_dir=$PWD
	cd $MAIN_DIR
	bash $ACCFLOW_TOOLS/begin_calcs.sh
	cd $curr_dir
}
...
function check_prog {
	python $ACCFLOW/utils/check_prog.py
}
...
function setup_flow {
	bash $ACCFLOW/utils/setup_flow.sh "$@"
}


# temporary functions
function rerun_sp_tddft {
	bash $ACCFLOW/scripts/rerun_sp_tddft.sh
}

function rerun_s1_opts {
	bash $ACCFLOW/scripts/sn-xyz-extract.sh
}
