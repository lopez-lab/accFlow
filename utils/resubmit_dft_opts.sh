...
function create_input_file {
	...
	local inchi="${pdb/_S0_SOLV.pdb/}"
    local curr_dir=$PWD

    if [[ "$curr_dir" == "$S0_SOLV" ]]; then
		route='#p B3LYP-D3BJ/4-31G* SCRF=(Solvent=Acetonitrile) opt'; loc=$S0_SOLV; title=$inchi\_S0_SOLV; s=1; c=0
	elif [[ "$curr_dir" == "$S1_SOLV" ]]; then
		route='#p B3LYP-D3BJ/4-31G* SCRF=(Solvent=Acetonitrile) opt td=root=1'; loc=$S1_SOLV; title=$inchi\_S1_SOLV; s=1; c=0
	elif [[ "$curr_dir" == "$T1_SOLV" ]]; then
		route='#p B3LYP-D3BJ/4-31G* SCRF=(Solvent=Acetonitrile) opt td=(triplets, root=1)'; loc=$T1_SOLV; title=$inchi\_T1_SOLV; s=3; c=0
    elif [[ "$curr_dir" == "$CAT_RAD_VAC" ]]; then
		route='#p B3LYP-D3BJ/4-31G* opt'; loc=$CAT_RAD_VAC; title=$inchi\_cat_rad_vac; s=2; c=1
    elif [[ "$curr_dir" == "$CAT_RAD_SOLV" ]]; then
		route='#p B3LYP-D3BJ/4-31G* SCRF=(Solvent=Acetonitrile) opt'; loc=$CAT_RAD_SOLV; title=$inchi\_cat_rad_solv; s=2; c=1
    fi
	bash $ACCFLOW/scripts/make-com.sh -i=$pdb_file -r="$route" -t=$title -c=$c -s=$s -l=$loc
}
