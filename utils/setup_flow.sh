...
for batch_title in "$@"; do
	...
	for d in pm7 rm1-d s0_solv; do mkdir $d/opt_pdbs; done
	for d in s0_vac s0_solv s1_solv t1_solv cat-rad_solv cat-rad_vac; do mkdir $d/completed $d/failed_opt $d/failed_freq $d/resubmits $d/freq_calcs $d/sp; done

	# setup config
	rsync -r --exclude=setup-flow.sh --exclude=functions.sh $ACCFLOW/* flow-tools/.
