...
if [[ $num_log_files > 0  ]]; then
	...
	for log_path in $log_files; do 
		...
		bash $ACCFLOW/scripts/make-com.sh -i=$log_path -r="#p M06/6-31+G(d,p) opt td=(root=1) SCRF=(Solvent=Acetonitrile)" -c=$(get_charge $inchi_key) -t=$title -f
	done
	submit_array "$TITLE\_S1_SOLV" "g16_inp.txt" "com" "$ACCFLOW_TOOLS/templates/array_g16_dft-opt.sbatch" "$DFT_TIME"
fi
